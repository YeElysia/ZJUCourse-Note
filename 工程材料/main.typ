...
#align(center)[
  ...
  #align(left)[
    ...
    本来学的时候是打算每周更新一点点认真学的，结果本笔记最后还是变成了补天笔记，6-8章选择了草草结束，大概是不会再更新了。好在最后两章除了要记牌号和选材三原则，剩下的内容考的并不多，重点还是以3-5章为主。
    #text(blue)[= 【成绩计算】]
    - 课堂点名 + 小测 #h(20pt)  #Red("10%")
    - 平时作业       #h(53pt)  #Red("30%")
    - 期末考试(闭卷)  #h(24pt)  #Red("60%")
    #text(blue)[= 【复习策略】]
    如果是短时间补天，请面向历年卷和ppt内的思考题进行复习，掌握重点内容甚至说记住答案即可。额外要掌握的仅有铁碳相图读法和选材三原则。

    如果时间较为充裕的话，建议从头到尾认真看一遍1-5章的内容，理解各个概念和原理，铁碳相图和C曲线的变化一定要记清楚，这个对理解后续一些内容很重要。四火记清各个温度范围和组织变化，回火三个温度的表格一定要记，这个必考。然后记住各个牌号代表了什么材料，数字是什么含义。 一般复习到这里我觉得就差不多能有80分了。

    如果还想再提高的话，可以把6-8章的内容也过一遍，了解一下常见材料的性能和用途。这样的话，请参考：

    纸鹭的笔记：https://www.cc98.org/topic/6032842（与回忆卷同一个帖子，笔记在3L）

    atri的笔记：https://www.cc98.org/topic/5931983

    我在本笔记的最后整理了ppt上的选择填空，以及常见牌号，仅供参考。

    我整理的2025-2026秋《工程材料》期末回忆卷在 https://www.cc98.org/topic/6345934
  ]
...
#align(center)[
  #table(
    ...
    [#text(red)[*体心立方晶胞*]], [$frac(sqrt(3), 4)a$], [2], [8], [0.68], [$alpha$-Fe, $delta$-Fe, Cr, W, Mo, V],
    [#text(red)[*面心立方晶胞*]], [$frac(sqrt(2), 4)a$], [4], [12], [0.74], [$gamma$-Fe, Ni, Al, Cu, Ag, Au],
...
#example()[
  ...
  #text(tips)[【解】] 冷拉钢丝绳是利用加工硬化效应提高其强度的，在这种状态下的钢丝中晶体缺陷密度增大，处于加工硬化状态。

  在900 ℃时保温，钢丝将发生回复、再结晶和晶粒长大过程,组织和结构恢复到软化状态。冷拉钢丝的加工硬化效果将消失，强度将比保温前低，再次起吊时，钢丝将被拉长，发生塑性变形并断裂。
...
- 碳素钢的在性能上主要有以下几方面的不足：
  ...
  - 回火稳定性差
  - 不能满足特殊性能的要求
- 合金钢：在钢中加入一定量的一种或几种元素以提高钢的某些性能，所加元素称为合金元素。
  1. 合金元素总量—低合金钢（合金元素总量小于5%），合金钢（合金元素总含量大于5%）。低合金钢与合金钢以合金元素含量多少区分时，两者合金元素规定含量界限值见表5-1。
  2. 合金钢按钢中所含主要合金元素分—锰钢，铬钢，铬钼钢或铬锰钛钢等。
  3. 低合金钢按质量等级分类—普通质量低合金钢、优质低合金钢、特殊质量低合金钢。

=== 合金钢的编号
我国的钢材编号是采用国际化学符号，数字及汉语拼音字母并用的原则，即钢中的合金元素用元素符号表示，如Si、 Mn、 Cr、Mo、Ni等，但稀土元素用“RE”表示其总含量。产品名称，用途，冶炼和浇注方法等用汉语拼音字母表示。
==== 合金结构钢

牌号：#text(blue)[60]#text(red)[Si2]#text(orange)[Mn]

#text(blue)[60：平均含碳量 $0.60 %$]（注意表示含碳量的数字有两位,为其玩分数）

#text(red)[Si2：含硅 2 %（平均含量$1.5 ~ 2.5 %$）]，如果是3表示含硅$3 %$（平均含量$2.5 ~ 3.5 %$），以此类推。

#text(orange)[Mn：含锰 1 %（平均含量低于 1.5 %，不标数字）]

#v(1em)
==== 滚动轴承钢
牌号: #text(blue)[G]#text(red)[Cr15]#text(orange)[Mn2]

#text(blue)[G：滚动轴承钢]

#text(red)[Cr15：含铬 $1.5 %$（注意这里的 Cr 是千分数，15 代表 1.5 %）]

#text(orange)[Mn2：含锰 $2 %$（Cr 之外的元素是百分数，2 代表 2 %）]
==== 合金工具钢
牌号：#text(blue)[9]#text(red)[Mn2V]

#text(blue)[9：平均含碳量 $0.9 %$（其实是 $0.85 ~ 0.95 %$）]（注意表示含碳量的数字只有一位,为其千分数）

#text(red)[Mn2V：与合金结构钢相同]

（若含碳量高于$1 %$，则不标含碳量，如 CrMn 平均含碳量则为$(1.3~1.5)%$，大于$1 %$）
==== 特殊性能钢
特殊性能钢具有特殊的物理化学性能，可分为不锈钢、耐热钢、耐磨钢和耐寒钢等。

牌号—这两类钢钢号前面的数字表示平均含碳量的千分数，如“9铬18”（即9Cr18）表示该钢平均含碳量为$0.9%$。但碳含量小于$0.03%$及$0.08%$者，在钢号前分别冠以“00”及“0”，如00Cr18Ni10等。

应指出，上述钢中能起重要作用的微量元素如钛，铌，锆等虽然含量较低（一般小于$1%$甚至小于$0.01%$），但应在钢号中标出，且元素符号后不附加数字；而当其在某些钢中含量大于$1%$时，则必须在元素符号后加上其含量的百分数。
...
合金元素与碳的亲和力从大到小的顺序为：Ti、Zr（锆）、Nb（铌）、V（钒）、W（钨）、Mo、Cr、Mn、Fe。

- Ti、Nb、V为强碳化物形成元素，碳化物的稳定性、熔点、硬度、耐磨性高，如TiC、VC等。
- W、Mo、Cr为中碳化物形成元素，碳化物的稳定性、熔点、硬度、耐磨性较高，如$W_2C$等。
- Mn、Fe为弱碳化物形成元素，碳化物的稳定性、熔点、硬度、耐磨性较低，如$F e_3C$等。
...
===== 对铁基固溶体的影响
碳钢中可能存在的基本相有铁素体、奥氏体和渗碳体。合金元素溶于固溶体类的相中，即形成合金铁素体、合金奥氏体，增加了固溶体相的稳定性，同时也对这些固溶体类相产生有效的固溶强化效果，非碳化物形成元素主要存在于固溶体类相中，#text(red)[韧性下降]。

===== 对碳化物的影响
形成合金碳化物或特殊碳化物。例如置换渗碳体中的铁原子，形成合金渗碳体[如(Fe,Cr)3C、(Fe,W)3C]等；在高碳高合金钢中，还可能形成各种稳定性更高的合金碳化物（如Mn3C、Cr7C3、Cr23C6等）以及特殊碳化物（WC、W2C、VC、TiC等），碳化物形成元素易于形成不同类型的碳化物。

稳定性愈高的碳化物愈难溶于奥氏体，愈难聚集长大。随着这些碳化物数量增多，将使钢的强度、硬度增大，耐磨性增加，但塑性和韧性会有所下降。

==== 合金元素对Fe - $F e_3C$相图的影响
===== 对奥氏体相区的影响
面心立方  Ni、Mn、Cu、C、N 等是扩大奥氏体相区的元素（溶解度大）。当Mn > $13%$或Ni > $9%$时，S点降到0 ℃以下，室温下为单相奥氏体组织，称奥氏体钢。

体心立方晶格 Cr、Mo、Si、Ti、W、Al等是缩小奥氏体相区的元素（溶解度大）。当含大量Cr时，奥氏体相区消失，室温下为单相铁素体组织，称铁素体钢。

===== 对E点和S点位置的影响
所有合金元素均使E点和S点左移，即这两点的含碳量下降，使碳含量比较低的钢出现过共析组织（如4Cr13）或共晶组织（如W18Cr4V）。

1）扩大A区合金元素—Mn、Co、Ni、C、Cu，奥氏体钢。

2）缩小A区合金元素—Mo、W、Ti、V、Cr、Si、Al，铁素体钢。
...
==== 合金元素对钢加热转变的影响。
1. 减慢奥氏体的形成速度。
2. 强烈阻碍奥氏体晶粒长大的作用，在合金钢中起细化晶粒的作用。
==== 合金元素对过冷奥氏体转变的影响。
1. C曲线右移
除Co以外，大多数合金元素总是不同程度地延缓珠光体和贝氏体相变，这是由于它们#text(blue)[溶入奥氏体后，增大其稳定性]，从而使C曲线右移。其中碳化物形成元素的影响最为显著。

2. 如果碳化物形成元素未能溶入奥氏体，而是以残存未溶碳化物微粒形式存在，则将起相反作用。

==== 合金元素对马氏体转变的影响。
1. $M_s$线和$M_f$线降低
除Co、Al外，大多数合金元素总是不同程度地降低马氏体转变温度（即$M_s$线和$M_f$线降低），并增加残余奥氏体量。因此可能需要多次回火。



==== 合金元素对淬火钢回火转变及性能的影响。
1. 合金钢的回火稳定性高
即合金钢在较高温度回火时仍然保持较高的硬度。
2. 二次淬火
某些高合金钢（如高速钢）中的残余奥氏体十分稳定，只有加热到500~600 ℃时才开始发生部分分解，从而使奥氏体的稳定性下降，在随后的快速冷却过程中转变为马氏体，使钢的硬度有较大提高。
3. 二次硬化
合金元素含量较高时，在一定的回火温度下，直接从马氏体中析出弥散分布、颗粒细小的特殊碳化物，使钢的硬度在该回火温度下显著升高。
#v(40em)
...
同时把密度低于$4.5×10^3 k g\/m^3$的金属称为轻金属。
== 铜及铜合金
=== 纯铜
=== 黄铜
==== 普通黄铜
...
加工特殊黄铜的牌号为：
H（黄）+主加元素符号（Zn除外）+铜平均百分含量+主加元素平均百分含量，如HPb59-1。
==== 白铜
...
==== 青铜
加工青铜的牌号为：Q +主加元素符号及其平均百分含量+其他元素平均百分含量。如QSn4-3（含4%Sn、3%Zn）。

===== 锡青铜
常用牌号有QSn4-3、QSn6.5-0.4、ZCuSn10Pb1（铸造青铜）等。
===== 铝青铜
常用牌号有QAl5、QAl7、ZCuAl8Mn13Fe3Ni2等。
===== 铍青铜
...
常用代号有ZL201 （ZAlCu5Mn）、ZL203 （ZAlCu4）等。主要用于制造在较高温度下工作的高强零件，如内燃机汽缸头、汽车活塞等。
...
==== 变形铝合金
变形铝合金的牌号以“铝”字的拼音首字“L”加“防”、“硬”、“超”、“锻”的拼音首字“F”、“Y”、“C”、“D”表示，分别为防锈铝合金（LF）、硬铝合金（LY）、超硬铝合金（LC）和锻铝合金（LD），后面数字为序号。

== 轴承合金
=== 常用轴承合金
工业上常用的是锡基、铅基轴承合金, 又称巴氏合金。
==== 锡基轴承合金
典型牌号为ZChSnSb11Cu6（其组织为基体+白亮块状SnSb+星状Cu3Sn）。
==== 铅基轴承合金
典型牌号为：ZChPbSb16Sn16Cu2。

== 钛及钛合金
=== 纯钛
=== 钛合金
按退火组织，钛合金可分为型钛合金、型钛合金和+型钛合金三类，它们的牌号分别用TA、TB、TC加顺序号表示 。如TA5、TB2、TC4等。其中TA0$~$TA3为工业纯钛。
...
= 工程材料的选用
== 选材的一般原则
- #text(red)[使用性原则]：保证零件完成规定功能的必要条件。
- #text(red)[工艺性原则]：材料的工艺性能表示材料加工的难易程度。
- #text(red)[经济性原则]：是选材的重要原则之一，一般包括材料的价格、零件的成本与国家的资源等。
...
==== 工程材料的强化方法
===== 固溶强化
是指由于晶格内融入异类原子而使材料强化的现象。溶质原子作为位错运动的障碍，增加了塑性变形抗力。原因主要为：
1）溶质原子引起晶格畸变，增加位错密度；
2）溶质原子与位错的交互作用。
===== 冷变形强化（加工硬化）
通过冷变形产生加工硬化是一种重要的强化手段。但冷变形强化后金属的塑性和韧性都有所下降。
===== 细晶强化（亦称晶界强化）
晶界的作用有两方面：1）阻碍位错运动；2）是位错聚集的地点。所以晶粒越细小，晶界面积越大，阻碍位错运动的障碍越多，位错密度也越大、越聚集，从而导致强度升高。细晶强化不但可提高强度，还可改善塑性和韧性。
===== 第二相强化
第二相强化是指利用合金中存在的第二相进行强化的现象。其机理与第二相的形态、数量及在基体上的分布方式有关。
1）分散强化    材料通过基体中分布有细小、弥散的第二相质点而产生强化的方法，称为分散强化。
2）双相合金中的第二相强化     当两相的体积和尺寸相差不大，结构、成分、性能相差较大时，欲使第二相起强化作用，应使第二相成片层状，最好是粒状分布。
===== 相变强化
相变强化主要是指马氏体强化（及下贝氏体强化），它是钢铁材料强化的重要途径。相变强化不是一种独立的强化方式，它是固溶强化、形变强化、细晶强化、第二相强化等多种强化效果的综合，是钢铁材料最经济而又最重要的一种强化途径。
...
3）承受一定的冲击载荷。
===== 主要失效形式
1）疲劳断裂：长期受扭转和弯曲交变载荷作用。
2）断裂失效：大载荷或冲击载荷作用。
3）磨损失效：轴颈或花键处过度磨损。
===== 对轴用材料性能要求
良好的综合力学性能；高的疲劳强度；良好的耐磨性。
==== 轴类零件的选材
===== 要求不高的轴
使用次数不多，载荷不大的低速转轴等，可选用价格低廉，性能一般的碳素结构钢，如Q235、Q255等。直接用圆钢加工而成，不再进行热处理。
===== 要求较高的轴
载荷较大，尺寸较大，中等转速，工作平稳的轴，如普通机床的主轴。这类轴要求材料具有良好的的综合力学性能，可选用中碳优质碳素结构钢，最常用的是45号钢。如使用滑动轴承，则颈部要求有较高的耐磨性，则可选用合金调质钢，最常用的是40Cr钢。
===== 要求更高的轴
===== 载荷不大，要求硬度高耐磨性好的小轴
===== 要求更高的轴
...
1. 网状渗碳体结构属于 #text(red)[二次] 渗碳体。消除网状渗碳体通常使用 #text(red)[正火] 热处理工艺。
2. 巴氏合金是 #text(red)[锡基] 和 #text(red)[铅基] 轴承合金。
3. 与 [321] 晶相（晶向）相互垂直的晶面为 #text(red)[321 晶面]。
4. 退火工艺的温度由高到低排序为：#text(red)[扩散退火] > #text(red)[完全退火] > #text(red)[球化退火] > #text(red)[去应力退火]
5. 耐磨钢常采用 #text(red)[ 表面渗碳淬火+低温回火] 热处理工艺。
6. 为使金属达到最高弹性极限，选用 #text(red)[中温回火] 热处理方式
7. 影响淬硬性的主要因素 #text(red)[马氏体含碳量]
8. 衡量金属塑性的指标是 #text(red)[伸长率 / 断面收缩率]
9. Mn和Cr对奥氏体区和B、E点的影响 #text(red)[扩大/缩小、左下/右下] （感觉是E,S点？）
10. 铁素体的力学性能特点是：强度 #text(red)[低]、塑性 #text(red)[好]、硬度 #text(red)[低]
11. 硬度最高的是 #text(red)[渗碳体] A.铁素体 B. 渗碳体 C. 珠光体 D. 奥氏体
12. 一次渗碳体是从 #text(red)[钢液] 中析出的。
13. 二次渗碳体是从 #text(red)[奥氏体] 中析出的。
14. 共析钢的过冷奥氏体在$550~350℃$温度区间等温转变形成的组织是 #text(red)[上贝氏体]
15. 共析钢的过冷奥氏体在$300℃$左右等温转变形成的组织是 #text(red)[下贝氏体]
16. 将碳钢缓慢加热到$500~600℃$，保温一段时间，然后缓冷的工艺为 #text(red)[去应力退火]
17. 为消除碳素工具钢中的网状渗碳体而进行正火，其加热温度应为 #text(red)[$A c_(c m)$+30～50℃]
18. 为消除金属在冷变形后的加工硬化现象，需进行的热处理为 #text(red)[再结晶退火]
19. 为使高碳钢便于机械加工，常预先进行 #text(red)[球化退火]
20. 钢经调质处理后获得的组织是 #text(red)[回火索氏体]
21. 淬火钢在$（300～400）℃$回火时，可达到最高值的机械性能是 #text(red)[弹性极限]
22. 淬火钢回火时，随着回火温度的升高 #text(red)[强度和硬度降低，塑性和韧性升高]
23. 过共析钢正常的淬火加热温度是 #text(red)[$A c_1+30～50℃$]
24. 量具的最终热处理一般为 #text(red)[淬火+低温回火]
25. 常用金属中，主要的晶格型式有 #text(red)[体心立方晶格] #text(red)[面心立方晶格] #text(red)[密排六方晶格]
26. 选材三种原则  #text(red)[使用性原则] #text(red)[工艺性原则] #text(red)[经济性原则]
27. 不锈钢的类型  #text(red)[奥氏体不锈钢] #text(red)[铁素体不锈钢] #text(red)[马氏体不锈钢]
28. HT200的200表示 #text(red)[抗拉强度]
29. 球化退火前常预先进行 #text(red)[正火] 热处理
30. 热变形的阶段：  #text(red)[回复] #text(red)[再结晶] #text(red)[晶粒长大]
31. 奥氏体化后的冷却方式是哪两种 #text(red)[等温转变、连续冷却转变]
32. 晶界发生滑移比晶体内部容易还是困难 #text(red)[困难]
33. 共析钢、过共析钢、亚共析钢中，哪个Ms线最低 #text(red)[过共析钢]，哪个C曲线最靠右 #text(red)[共析钢]
34. 影响淬透性最大的因素 #text(red)[合金元素]
#v(100em)
#align(center)[
...
#figure(image("pic/牌号.jpg", width: 95%))
