...
当输入信号变化较小时，可认为晶体管的工作点Q基本不变，因此可以用一个线性模型来近似晶体管的输入特性曲线。此时有动态电阻$r_(b e) = (Delta U_(B E))/(Delta I_B)$。常温下：
$
  r_(b e) = r_b (200 Omega) + (1+beta) 26/I_E = r_b + 26/I_B (I"单位"m A)
$
由此可以画出#Red[对变化量而言]的电路模型(c)。
#callouts.Note(title: [r$""_(b e)$如何得到])[
  (来源于gemini) 基极到发射极的压降主要来自于基区的电阻和发射结的动态电阻。基区电阻$r_b$是一个常数，典型值为200Ω；发射结的动态电阻$r_e$发射结这个 PN 结在交流信号下的阻抗。因此我们得到
  $
